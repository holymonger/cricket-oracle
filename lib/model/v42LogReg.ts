...
let cachedArtifact: V42LogRegArtifact | null = null;
...
function resolveArtifactPath(): string {
  const filename = "v42_logreg.json";
  const candidates = [
...
function loadArtifact(): V42LogRegArtifact {
  if (cachedArtifact) return cachedArtifact;

  const artifactPath = resolveArtifactPath();
  if (!fs.existsSync(artifactPath)) {
    throw new Error(`v42-logreg artifact not found at ${artifactPath}. Run: npm run train:v42logreg`);
  }

  const raw = fs.readFileSync(artifactPath, "utf-8");
  const parsed = JSON.parse(raw) as V42LogRegArtifact;
  ...
  if (!Array.isArray(parsed.featureNames) || !Array.isArray(parsed.coeff)) {
    throw new Error("invalid v42-logreg artifact shape");
  }
...
export function predictFromFeaturesV42(featureRow: Record<string, number>): WinProbResult {
  ...
  const calibratedProb = calibrateProb(rawProb, "v42-logreg");
  return {
    winProb: calibratedProb,
    modelVersion: "v42-logreg",
    features: {
      ...featureRow,
      __rawProb: rawProb,
      __calibratedProb: calibratedProb,
    },
  };
}

export function computeWinProbV42LogReg(
  state: MatchState,
  ...
  return predictFromFeaturesV42(fallbackFeatures);
}
